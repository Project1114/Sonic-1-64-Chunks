...
namespace SonicRetro.SonLVL.API.S1D
{
	public class Layout : LayoutFormatCombined
	{
			...
			for (int la = 0; la < Math.Max(fgh, bgh) * 4; la += 4)
			{
				...
				if (lfp != 0)
					for (int laf = 0; laf < fgw; laf++)
						layout.FGLayout[laf, la / 4] = ByteConverter.ToUInt16(rawdata, lfp - StartAddress + laf * 2);
				ushort lbp = ByteConverter.ToUInt16(rawdata, 8 + la + 2);
				if (lbp != 0)
					for (int lab = 0; lab < bgw; lab++)
						layout.BGLayout[lab, la / 4] = ByteConverter.ToUInt16(rawdata, lbp - StartAddress + lab * 2);
			}
		}
		...
		public override void WriteLayout(LayoutData layout, out byte[] rawdata)
		{
			...
			{
				if (la < fgh)
					tmp.AddRange(ByteConverter.GetBytes((ushort)(StartAddress + 0x108 + (la * fgw * 2))));
				else
					tmp.AddRange(new byte[2]);
				if (la < bgh)
					tmp.AddRange(ByteConverter.GetBytes((ushort)(StartAddress + 0x108 + (fgh * fgw * 2) + (la * bgw * 2))));
				else
					tmp.AddRange(new byte[2]);
			}
			for (int y = 0; y < fgh; y++)
	...
	//	public override bool HasXFlipFlag { get { return true; } }
		
	//	public override bool HasYFlipFlag { get { return true; } }

		public override bool IsResizable { get { return true; } }

		public override System.Drawing.Size MaxSize { get { return new System.Drawing.Size(400, 64); } }

		public override System.Drawing.Size DefaultSize { get { return new System.Drawing.Size(256, 32); } }

		public virtual ushort StartAddress { get { return 0x0000; } }

	//public override int MaxBytes => (0x8000-0x88);
    }
}
